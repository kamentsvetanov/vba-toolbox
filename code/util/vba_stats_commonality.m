...
% try estimationMethod = cfg.estimationMethod; catch estimationMethod = 'QR'; end 
% % Options: 'QR'    - fast, by hand, non-robust only
% %          'fitlm' - full fitlm object, supports robust, categorical interactions

doRobust    = cfg.doRobust;
estimationMethod = cfg.estimationMethod;
...
for i = 1:numcc
    ...
    for j = 2:nvar
        ...
        if bit == 1
            alist = ilist;
            blist = vba_stats_commonality_genlist(ilist, -ivID{j,1});
            ilist = [alist blist];
        else
            ilist = vba_stats_commonality_genlist(ilist, ivID{j,1});
        end
    end
    ilist = ilist .* -1;
...
%     APSMatrix = array2table(nan(numcc, 2),'RowNames',cellstr(Rownames),'VariableNames',{'k','R2'});
    ...
    switch estimationMethod
        case 'QR' % LAPACK's Least-Squares Solver. fast, by hand, non-robust only
            y           = dataMatrix.(dv);
            mu_y        = mean(y);
            APSMatrix = vba_stats_commonality_estimateAPSMatrix_fast(dataTemp,ModelPred,numcc,Ns,y,mu_y);
        case 'fitlm'
            if doRobust == 1
                robustOpts = 'Huber';
            else
            ...
            APSMatrix = vba_stats_commonality_estimateAPSMatrix_mlr(dataTemp,Model,numcc,robustOpts);
    end
